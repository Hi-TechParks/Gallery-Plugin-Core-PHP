...
function get_theme_before($loopvars, $theme_settings, $core_settings) {
	...
	$temp_content.="<a href='{$loopvars->get_var("gallery_listing_link")}' {$loopvars->get_var("link_target")} class='gallery_home_nav button'>Galleries</a>";
	
	
	if($core_settings['allow_user_registration'] && !(isset($_SESSION['s_userName'])) && !$loopvars->get_var("wordpress") ){
		$temp_content.="<a href='{$loopvars->get_var("registration_link")}' {$loopvars->get_var("link_target")} class='registration_nav button'>Register</a>";
	}
	...
	if(isset($_SESSION['s_userName']) && !$loopvars->get_var("wordpress") ){
		...
		if( can_admin() ){
			//Settings link
			//$temp_content.="<a href='{$loopvars->get_var("edit_config_link")}' {$loopvars->get_var("link_target")} class='edit_users_nav button'>Settings</a>";
			$temp_content.="<a href='{$loopvars->get_var("edit_users_link")}' {$loopvars->get_var("link_target")} class='edit_users_nav button'>Users</a>";
		}
	}else{
	...
	if( (is_logged_in() && !strrpos($loopvars->get_var("current_url"), '/admin/index.php') ) || (is_logged_in() && strrpos($loopvars->get_var("current_url"), '/admin/index.php') && !$loopvars->compare("action", "")) ){
		$temp_content.="<a href='{$loopvars->get_var("manage_link")}' class='manage_home_nav button' ".$loopvars->get_var("manage_link_target").">Manage</a>";
	}
	...
	if(!$loopvars->compare("action", "vg") && !$loopvars->compare("action", "vi") && !$loopvars->compare("action", "reg") && !$loopvars->compare("action", "pr") && !$loopvars->compare("action", "chgpass") && !strrpos($loopvars->get_var("current_url"), '/admin/index.php') && !strrpos($loopvars->get_var("current_url"), '/admin') && !strrpos($loopvars->get_var("current_url"), '/admin/') && !$loopvars->compare("personal_gallery_user", "")){
				...
				$temp_content.="<div class='personal_gallery_label'>Viewing <a href='{$loopvars->get_var("personal_gallery_link_gallery_tile")}' {$loopvars->get_var("link_target")}>{$loopvars->get_var("personal_gallery_user")}</a>'s galleries <a href='{$loopvars->get_var("base_url")}' {$loopvars->get_var("link_target")} class='personal_gallery_return_all button'>return to all galleries</a></div>";
	}
	...
	if(!$loopvars->compare("gallery_name", "") && ($loopvars->compare("action", "vg") || $loopvars->compare("action", "vi")) ){
			...
			$temp_content.="<div class='gallery_label'>Viewing <a href='{$loopvars->get_var("gallery_link")}' title='{$loopvars->get_var("gallery_description")}' {$loopvars->get_var("link_target")}>{$loopvars->get_var("gallery_title")}</a> posted by <a href='{$loopvars->get_var("personal_gallery_link_gallery_tile")}' title='click to view more galleries by {$loopvars->get_var("gallery_poster")}' {$loopvars->get_var("link_target")}>{$loopvars->get_var("gallery_poster")}</a> on {$loopvars->get_var("gallery_date_posted")}<div class='vg_top_admin_options'>{$loopvars->get_var("edit_gallery_block")}{$loopvars->get_var("upload_files_block")}{$loopvars->get_var("delete_gallery_block")}{$loopvars->get_var("edit_collaborators_block")}</div></div>";
	}

	if($loopvars->compare("gallery_name", "") && $loopvars->compare("action", "s") ){
		$temp_content.="<div class='search_label'>Search results for: <span>".htmlentities($loopvars->get_var("search_term"))."</span></div>";
	}
	...
	if($loopvars->compare("action", "vi")){
		...
		if($loopvars->get_var("previous_image_selector")){
			$temp_content.="<a href='{$loopvars->get_var("previous_image_link")}' {$loopvars->get_var("link_target")} class='previous_image_nav button'>Prev</a>";
		}
		
		$temp_content.="<a href='{$loopvars->get_var("back_to_gallery_link")}' {$loopvars->get_var("link_target")} class='back_to_thumbs_nav button'>Back To Thumbnails</a>";
	
		if($loopvars->get_var("next_image_selector")){
	 		$temp_content.="<a href='{$loopvars->get_var("next_image_link")}' {$loopvars->get_var("link_target")} class='next_image_nav button'>Next</a>";
		}
	    ...
	    $previouspage=str_replace("&lt;", "<span class='prev_page_nav button'>Prev</span>", $loopvars->get_var("previous_page_selector"));
	    $nextpage=str_replace("&gt;", "<span class='prev_page_nav button'>Next</span>", $loopvars->get_var("next_page_selector"));
...
function get_theme_gallery_tile($loopvars, $theme_settings, $core_settings) {
	...
	$temp_content.="<div class='gallery_thumbnail'><a href='{$loopvars->get_var("gallery_link")}' {$loopvars->get_var("link_target")}><img alt='' src='".$loopvars->get_var("thumbnail_media_path")."' border='0' /></a></div>";
	...
	if($theme_settings->get_var("show_poster_name")){
		...
		$temp_content.="<p class='gallery_posted_by'>Posted By: ";
		
		if($loopvars->get_var("personal_gallery_user")!=$loopvars->get_var("gallery_poster")){
			
			$temp_content.="<span><a href='{$loopvars->get_var("personal_gallery_link_gallery_tile")}' {$loopvars->get_var("link_target")}>{$loopvars->get_var("gallery_poster")}</a></span";
		
		}else{
			
			$temp_content.="<span>".$loopvars->get_var("gallery_poster")."</span>";
		}
	...
	if($theme_settings->get_var("show_posted_date")){
		...
		$temp_content.="<p class='gallery_posted_date'>Posted On: <span>{$loopvars->get_var("gallery_date_posted")}</span></p>";

	}

	if(!$loopvars->compare("gallery_copyright", "")){

		$temp_content.="<p class='gallery_copyright'>Copyright: <span>{$loopvars->get_var("gallery_copyright")}</span></p>";

	}

	$temp_content.="</div>";
	
	$temp_content.="<div class='owner_options'>".$loopvars->get_var("edit_gallery_block")." ".$loopvars->get_var("upload_files_block")." ".$loopvars->get_var("delete_gallery_block")." ".$loopvars->get_var("edit_collaborators_block")."</div>";
